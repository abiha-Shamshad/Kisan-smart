...
def register_user():
    ...
    if User.query.filter_by(email=data['email']).first():
        return error_response("Email already registered", "EMAIL_ALREADY_EXISTS", None, 400)
    ...
    return success_response(user_schema.dump(new_user), "User registered successfully", 201)
...
def login_user():
    ...
    if user and bcrypt.check_password_hash(user.password_hash, data['password']):
        if not user.is_active:
            return error_response("Account is deactivated", "ACCOUNT_DEACTIVATED", None, 403)
        if user.is_locked:
...
def forgot_password():
    return success_response(None, "Password reset email sent if account exists (Placeholder)")
