...
def run_pipeline(version="v1.0.0"):
    ...
    X_train, X_val, X_test, y_train_type, y_val_type, y_test_type = loader.split_data(
        target_col="Fertilizer"
    )

    # Then Regression (Fertilizer Quantity)
    _, _, _, y_train_quant, y_val_quant, y_test_quant = loader.split_data(
        target_col="Quantity"
    )
    ...
    evaluator.save_report(
        y_val_type_enc,
        best_clf.predict(X_val_transformed),
        "best_classifier",
        mode="classification",
    )
    ...
    evaluator.plot_roc_curve(
        best_clf, X_val_transformed, y_val_type_enc, "best_classifier", labels_enc
    )

    # Regression Plots for Best Model
    y_pred_quant = best_reg.predict(X_val_transformed)
    evaluator.save_report(
        y_val_quant, y_pred_quant, "best_regressor", mode="regression"
    )
