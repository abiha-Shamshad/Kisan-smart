...
def generate_synthetic_data(num_samples=10000):
    ...
    for i in range(num_samples):
        ...
        if n < 50 and p > 60:
            ...
            fert = 'Urea'
            base_q = 40 + (n - 100) * 0.3 # More N excess? No, Urea is for N deficiency usually.
        ...
        stage_multipliers = {'Initial': 0.8, 'Vegetative': 1.2, 'Flowering': 1.0, 'Maturity': 0.5}
    ...
    for col in ['Nitrogen', 'pH', 'Moisture']:
        indices = np.random.choice(df.index, size=int(num_samples * 0.05), replace=False)
