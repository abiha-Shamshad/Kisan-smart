...
class KisanDataPipeline:
    def __init__(
        self,
        raw_path="data/raw/agricultural_data.csv",
    ...
    def exploratory_data_analysis(self, output_dir="static/plots"):
        ...
        sns.heatmap(
            self.df[numerical_cols].corr(), annot=True, cmap="coolwarm", fmt=".2f"
        )
    ...
    def clean_data(self):
        ...
        # Capping outliers
        self.df["Nitrogen"] = np.where(
            self.df["Nitrogen"] > upper_bound,
            upper_bound,
            np.where(
                self.df["Nitrogen"] < lower_bound, lower_bound, self.df["Nitrogen"]
            ),
        )
    ...
    def transform_data(self):
        ...
        print("\n--- Transforming Data ---")
        # Example: NPK Ratio
        self.df["Total_Nutrients"] = (
            self.df["Nitrogen"] + self.df["Phosphorus"] + self.df["Potassium"]
        )
