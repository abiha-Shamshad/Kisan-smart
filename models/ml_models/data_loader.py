...
class DataLoader:
    ...
    def load_data(self):
        ...
        if missing > 0:
            print(
                f"Warning: {missing} missing values detected. These should be handled in preprocessing."
            )
    ...
    def split_data(
        ...
        X_val, X_test, y_val, y_test = train_test_split(
            X_rem,
            y_rem,
            test_size=test_ratio_of_rem,
            random_state=random_state,
            stratify=stratify_rem,
        )

        print(
            f"Data split ratios for {target_col}: Train({len(X_train)}), Val({len(X_val)}), Test({len(X_test)})"
        )
